-- ============================================
-- KORY DARK UI SUITE - ANA DOSYA
-- Developed by Kory Labs
-- Version: Stable Build 3.0
-- ============================================
...
--[[
    ...
    Main Credits
    Kory (Kory Labs) | Lead Developer & Design
    Shadow (Kory Labs) | Co-Developer & Bug Fixes
    Nexus | UI Components & Color Engine
    Zephyr | Animation & Event Handling
    Kory Executor | Original UI Concept

    Special Thanks To
    Lynx | Beta Testing
    Eris | Configuration Module
    Altitude Studios & Apex | Icon Resources
    Vertex | Loading Animations
    Kael | Script Optimization & Testing
]]

local Release = "Stable Build 3.0"
...
local isStudio
local website = "github.com/Kory-Labs"
...
local IconModule = {
    ...
    Material = {
        ...
        ["dark_mode"] = "rbxassetid://6031094675",
        ["light_mode"] = "rbxassetid://6031094687",
...
local function GetIcon(icon, source)
    ...
    elseif source == "Lucide" then
        return IconModule.Material[icon] or "rbxassetid://6031097225"
    else
        return IconModule.Material[icon] or "rbxassetid://6031097225"
    end
end
...
-- ANA UI NESNELERİ
local KoryUI = isStudio and script.Parent:WaitForChild("Kory UI") or game:GetObjects("rbxassetid://86467455075715")[1]
local SizeBleh = nil

if gethui then
    KoryUI.Parent = gethui()
elseif syn and syn.protect_gui then 
    syn.protect_gui(KoryUI)
    KoryUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
    KoryUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
    for _, Interface in ipairs(gethui():GetChildren()) do
        if Interface.Name == KoryUI.Name and Interface ~= KoryUI then
            Hide(Interface.SmartWindow)
            Interface.Enabled = false
            Interface.Name = "Kory-Old"
        end
    end
elseif not isStudio then
...
KoryUI.Enabled = false
KoryUI.SmartWindow.Visible = false
KoryUI.Notifications.Template.Visible = false
KoryUI.DisplayOrder = 1000000000

local Main : Frame = KoryUI.SmartWindow
local Dragger = Main.Drag
local dragBar = KoryUI.Drag
...
local Notifications = KoryUI.Notifications
local KeySystem : Frame = Main.KeySystem

-- BİLDİRİM FONKSİYONU
function Kory:Notification(data)
    task.spawn(function()
        data = Kwargify({
            Title = "Missing Title",
...
-- KEY SYSTEM HANDLER
local function KeySystemHandler(WindowSettings)
    ...
    local direc = KeySettings.SaveInRoot and "Kory/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "Kory/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

    if isfile and isfile(direc .. KeySettings.FileName .. ".kory") then
        for i, Key in ipairs(KeySettings.Key) do
            if string.find(readfile(direc .. KeySettings.FileName .. ".kory"), Key) then
                return true
            end
        end
    end
    ...
    KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
        ...
        if KeyFound then 
            ...
            if KeySettings.SaveKey then
                if writefile then
                    writefile(direc .. KeySettings.FileName .. ".kory", FoundKey)
                end
                Kory:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
            end
            return true
        else
...
-- ============================================
-- CREATEWINDOW FONKSİYONU
-- ============================================
function Kory:CreateWindow(WindowSettings)

    WindowSettings = Kwargify({
        Name = "Kory UI Example Window",
        Subtitle = "",
        LogoID = "6031097225",
        LoadingEnabled = true,
        LoadingTitle = "Kory Interface Suite",
        LoadingSubtitle = "by Kory Labs",
        ConfigSettings = {},
        KeySystem = false,
        KeySettings = {}
    }, WindowSettings or {})
    ...
    LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "Kory Interface Suite" and Release or "Kory UI"
    ...
    if WindowSettings.KeySystem then
        Passthrough = KeySystemHandler(WindowSettings)
    ...
    Draggable(KoryUI.MobileSupport, KoryUI.MobileSupport)
    ...
    function Window:CreateHomeTab(HomeTabSettings)
        HomeTabSettings = Kwargify({
            Icon = 1,
            SupportedExecutors = {"Kory X", "Dark Executor", "Shadow", "Nexus"},
        ...
        for i,v in pairs(HomeTabSettings.SupportedExecutors) do
            if isStudio then 
                HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "Kory Interface Suite - Debugging Mode" 
                break 
            end
    ...
    function Window:CreateTab(TabSettings)
        ...
        function Tab:CreateSection(name : string)
            ...
            function Section:CreateButton(ButtonSettings)
                ...
                Button.Interact["MouseButton1Click"]:Connect(function()
                    ...
                    if not Success then
                        ...
                        print("Kory UI | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
                        wait(0.5)
                        Button.Title.Text = ButtonSettings.Name
                        TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
            ...
            function Section:CreateSlider(SliderSettings, Flag)
                        ...
                        if SLDragging then
                            ...
                            if SliderSettings.CurrentValue ~= NewValue then
                                ...
                                if not Success then
                                    ...
                                    print("Kory UI | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
                                    wait(0.5)
                                    Slider.Title.Text = SliderSettings.Name
                                    TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
                ...
                if Flag then
                    Kory.Options[Flag] = SliderV
                end

                KoryUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
                    Slider.Main.color.Color = Kory.ThemeGradient
                    Slider.Main.UIStroke.color.Color = Kory.ThemeGradient
                end)
            ...
            function Section:CreateToggle(ToggleSettings, Flag)    
                ...
                Toggle.Interact.MouseButton1Click:Connect(function()
                    ...
                    if not Success then
                        ...
                        print("Kory UI | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
                        wait(0.5)
                        Toggle.Title.Text = ToggleSettings.Name
                        TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
                ...
                KoryUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
                    Toggle.toggle.color.Color = Kory.ThemeGradient
                    Toggle.toggle.UIStroke.color.Color = Kory.ThemeGradient
                end)

                if Flag then
                    Kory.Options[Flag] = ToggleV
                end
                    ...
                    if CheckingForKey then
                        if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
                            ...
                            if not Success then
                                ...
                                print("Kory UI | "..BindSettings.Name.." Callback Error " ..tostring(Response))
                                wait(0.5)
                                Bind.Title.Text = BindSettings.Name
                    ...
                    if InputSettings.Enter then
                        if bleh then
                            ...
                            if not Success then
                                ...
                                print("Kory UI | "..InputSettings.Name.." Callback Error " ..tostring(Response))
                                wait(0.5)
                                Input.Title.Text = InputSettings.Name
            ...
            function Section:CreateDropdown(DropdownSettings, Flag)
                ...
                local function SafeCallback(param, c2)
                    ...
                    if not Success then
                        ...
                        print("Kory UI | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
                        wait(0.5)
                        Dropdown.Title.Text = DropdownSettings.Name
            ...
            function Section:CreateColorPicker(ColorPickerSettings, Flag)
                ...
                local function SafeCallback(param, c2)
                    ...
                    if not Success then
                        ...
                        print("Kory UI | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
                        wait(0.5)
                        ColorPicker.Title.Text = ColorPickerSettings.Name
                ...
                if Flag then
                    Kory.Options[Flag] = ColorPickerV
                end
        ...
        function Tab:BuildConfigSection()
            ...
            Tab:CreateButton({
                ...
                Callback = function()
                    ...
                    local success, returned = Kory:SaveConfig(inputPath)
                    if not success then
                        Kory:Notification({
                            Title = "Interface",
                            Icon = "error",
                    ...
                    configSelection:Set({ Options = Kory:RefreshConfigList() })
                end
            })
            ...
            configSelection = Tab:CreateDropdown({
                ...
                Options = Kory:RefreshConfigList(),
                CurrentOption = {},
                MultipleOptions = false,
                SpecialType = nil,
            ...
            Tab:CreateButton({
                ...
                Callback = function()
                    local success, returned = Kory:LoadConfig(selectedConfig)
                    if not success then
                        Kory:Notification({
                            Title = "Interface",
                            Icon = "error",
            ...
            Tab:CreateButton({
                ...
                Callback = function()
                    local success, returned = Kory:SaveConfig(selectedConfig)
                    if not success then
                        Kory:Notification({
                            Title = "Interface",
                            Icon = "error",
            ...
            Tab:CreateButton({
                ...
                Callback = function()
                    ...
                    writefile(Kory.Folder .. "/settings/autoload.txt", name)
            ...
            Tab:CreateButton({
                ...
                Callback = function()
                    local name = selectedConfig
                    delfile(Kory.Folder .. "/settings/autoload.txt")
            ...
            if isfile(Kory.Folder .. "/settings/autoload.txt") then
                local name = readfile(Kory.Folder .. "/settings/autoload.txt")
                loadlabel:Set( { Text = "Current autoload config: " .. name })
            end     
        end
        ...
        function Tab:BuildThemeSection()
            ...
            }, "KoryInterfaceSuitePrebuiltCPC1")

            local c2cp = Tab:CreateColorPicker({
                Name = "Color 2",
                Color = Color3.fromRGB(30, 30, 30),
            }, "KoryInterfaceSuitePrebuiltCPC2")

            local c3cp = Tab:CreateColorPicker({
                Name = "Color 3",
                Color = Color3.fromRGB(15, 15, 15),
            }, "KoryInterfaceSuitePrebuiltCPC3") 
            ...
            c1cp:Set({
                Callback = function(Value)
                    if c2cp and c3cp then
                        Kory.ThemeGradient = ColorSequence.new{
                            ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), 
                            ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), 
                            ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))
                        }
                        KoryUI.ThemeRemote.Value = not KoryUI.ThemeRemote.Value
                    end
                end
            })
            ...
            c2cp:Set({
                Callback = function(Value)
                    if c1cp and c3cp then
                        Kory.ThemeGradient = ColorSequence.new{
                            ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), 
                            ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), 
                            ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))
                        }
                        KoryUI.ThemeRemote.Value = not KoryUI.ThemeRemote.Value
                    end
                ...
                Callback = function(Valuex)
                    if c2cp and c1cp then
                        Kory.ThemeGradient = ColorSequence.new{
                            ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), 
                            ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), 
                            ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))
                        }
                        KoryUI.ThemeRemote.Value = not KoryUI.ThemeRemote.Value
                    end
                end
            ...
            local PresetGradients = {
                ["Dark Night"] = {Color3.fromRGB(45, 45, 45), Color3.fromRGB(30, 30, 30), Color3.fromRGB(15, 15, 15)},
                ["Obsidian"] = {Color3.fromRGB(60, 60, 60), Color3.fromRGB(40, 40, 40), Color3.fromRGB(20, 20, 20)},
                ["Shadow"] = {Color3.fromRGB(80, 80, 80), Color3.fromRGB(50, 50, 50), Color3.fromRGB(25, 25, 25)},
                ["Blood Moon"] = {Color3.fromRGB(100, 20, 20), Color3.fromRGB(70, 10, 10), Color3.fromRGB(40, 5, 5)},
                ["Deep Sea"] = {Color3.fromRGB(20, 40, 80), Color3.fromRGB(15, 30, 60), Color3.fromRGB(10, 20, 40)},
                ["Forest Dark"] = {Color3.fromRGB(30, 60, 30), Color3.fromRGB(20, 45, 20), Color3.fromRGB(10, 30, 10)},
            }
    ...
    Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
        ...
        if UserInputService.KeyboardEnabled == false then
            KoryUI.MobileSupport.Visible = true
        end
    end)
    ...
    UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        if input.KeyCode == Window.Bind then
            ...
            KoryUI.MobileSupport.Visible = false
            dragBar.Visible = true
            Window.State = true
        end
    end)
    ...
    KoryUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
        Unhide(Main, Window.CurrentTab)
        dragBar.Visible = true
        Window.State = true
        KoryUI.MobileSupport.Visible = false
...
function Kory:Destroy()
    ...
    local blurFolder = workspace.CurrentCamera:FindFirstChild("KoryBlur")
    if blurFolder then
        blurFolder:Destroy()
    end
    
    for _, effect in ipairs(game:GetService("Lighting"):GetChildren()) do
        if effect:IsA("DepthOfFieldEffect") and effect.Name:match("DPT_") then
            effect:Destroy()
        end
    end
    
    collectgarbage()
    print("Kory Dark UI Suite - Destroyed Successfully")
end

-- ============================================
-- CONFIG YÖNETİM FONKSİYONLARI
-- ============================================
local ClassParser = {
    ["Toggle"] = {
        ...
        Load = function(Flag, data)
            if Kory.Options[Flag] then
                Kory.Options[Flag]:Set({ CurrentValue = data.state })
            end
        end
    },
    ["Slider"] = {
        Save = function(Flag, data)
            return { type = "Slider", flag = Flag, value = (data.CurrentValue and tostring(data.CurrentValue)) }
        end,
        Load = function(Flag, data)
            if Kory.Options[Flag] and data.value then
                Kory.Options[Flag]:Set({ CurrentValue = data.value })
            end
        end
    },
    ["Input"] = {
        Save = function(Flag, data)
            return { type = "Input", flag = Flag, text = data.CurrentValue }
        end,
        Load = function(Flag, data)
            if Kory.Options[Flag] and data.text and type(data.text) == "string" then
                Kory.Options[Flag]:Set({ CurrentValue = data.text })
            end
        end
    },
    ["Dropdown"] = {
        Save = function(Flag, data)
            return { type = "Dropdown", flag = Flag, value = data.CurrentOption }
        end,
        Load = function(Flag, data)
            if Kory.Options[Flag] and data.value then
                Kory.Options[Flag]:Set({ CurrentOption = data.value })
            end
        end
    },
    ["Colorpicker"] = {
        Save = function(Flag, data)
            ...
            return { type = "Colorpicker", flag = Flag, color = Color3ToHex(data.Color) or nil }
        end,
        Load = function(Flag, data)
            ...
            if Kory.Options[Flag] and data.color then
                Kory.Options[Flag]:Set({Color = HexToColor3(data.color)})
            end
        end
    }
}

function Kory:SaveConfig(Path)
    if isStudio then return "Config system unavailable." end
    if (not Path) then return false, "Please select a config file." end

    local fullPath = Kory.Folder .. "/settings/" .. Path .. ".kory"
    local data = { objects = {} }

    for flag, option in next, Kory.Options do
...
function Kory:LoadConfig(Path)
    if isStudio then return "Config system unavailable." end
    if (not Path) then return false, "Please select a config file." end

    local file = Kory.Folder .. "/settings/" .. Path .. ".kory"
...
function Kory:LoadAutoloadConfig()
    if isfile(Kory.Folder .. "/settings/autoload.txt") then
        if isStudio then return "Config system unavailable." end
        local name = readfile(Kory.Folder .. "/settings/autoload.txt")
        local success, err = Kory:LoadConfig(name)
        if not success then
            return Kory:Notification({
                Title = "Interface",
                Icon = "sparkle",
                ImageSource = "Material",
...
function Kory:RefreshConfigList()
    if isStudio then return "Config system unavailable." end
    local list = listfiles(Kory.Folder .. "/settings")
    local out = {}
    for i = 1, #list do
        local file = list[i]
        if file:sub(-5) == ".kory" then
            local pos = file:find(".kory", 1, true)
            local start = pos
...
local function BuildFolderTree()
    if isStudio then return "Config system unavailable." end
    local paths = { Kory.Folder, Kory.Folder .. "/settings" }
    for i = 1, #paths do
        local str = paths[i]
...
local function SetFolder()
    ...
    if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
        Kory.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
    else
        Kory.Folder = WindowSettings.ConfigSettings.ConfigFolder
    end
    BuildFolderTree()
end

SetFolder()
Kory:LoadAutoloadConfig()
